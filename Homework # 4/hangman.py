...
def set_words(word_len: int, filename: str = "hangman_list.txt") -> typing.Set[str]:
...
def check_same_items_in_list(list): # fuction for comparison if all items in list are the same
    ...
    try:
        while is_playing:
            if was_correct:
                ...
                len_list.append(len(current_word)) # list for collecting length of input string

                check_word = [s for s in current_word if s.isalpha()] # list for check does computer win?
                if check_same_items_in_list(len_list) is False:
                    print('В предыдущих ходах ты вводил слово с другой длиной.')
                    len_list.pop()
                    continue
                if len(current_word) == len(check_word) and len(current_word)>0: # computer winning condition
                    if current_word != possible_words[0] and len(possible_words) == 1:
                        print(f'Ты пытаешся мене обмануть, но я то знаю, что правильное слово {possible_words[0]}.')
                        print(f"Это было не сложно, мне понадобилось всего {len(guesses)} попыток!")
                        break
                    elif current_word == possible_words[0]:
                        print(f"Угаданное слово {possible_words[0]}.")
                        print(f"Это было не сложно, мне понадобилось всего {len(guesses)} попыток!")
                        break
                    else:
                        print(f'Ты мне даешь невалидную информацию. Попробуй еще.')
                        continue

                if check_letter not in current_word:
                    print('Введи символ, который я угадал. ')
                    continue
                check_user_input = [s for s in current_word if s.isalpha() or s=='_'] # list for check does user enter correctly?
                if attempt == 1: # attempt value for entering the word by user from the first attempt
                    if len(current_word.strip('_')) > 0:
                        print('Я еще ничего не угадывал. Зачем ты мне уже сейчас даешь подсказки? Давай сначала.')
                        attempt = 0
                        play_game()
                if len(check_user_input) != len(current_word) and  check_user_input.count('_')>0:
                    print(" Ты загадал слово с невалидными символами. Попробуй еще.")
                    play_game()
                elif len(current_word) == 0:
                    print("Ты ничего не написал. Попробуй снова.")
                    play_game()
                else:
            ...
            if was_correct:
                check_letter = likeliest_letter[0]
            guesses += likeliest_letter[0]

    except IndexError:
        print('Очевидно, твоего слова не существует в списке игры. Попробуй еще раз')
